"""
VB-Audio Virtual Cable Switcher

Manages switching Windows default playback device to VB Cable
using PowerShell AudioDeviceCmdlets module.
"""
import subprocess
import time
...
class VB_CableSwitcher:
    """
    Manages switching Windows default playback device to VB Cable.
    Uses PowerShell AudioDeviceCmdlets module to change default audio device.
    """
    
    # PowerShell command templates (reduces string duplication)
    ...
    def _run_powershell_command(self, command: str, timeout: int = 5) -> Optional[str]:
        ...
        This is a DRY helper that centralizes all PowerShell execution logic,
        reducing code duplication and ensuring consistent error handling.
        
        Args:
            command: PowerShell command to execute (without 'powershell -NoProfile -Command')
    ...
    def get_current_default_device(self) -> Optional[str]:
        ...
        # Try multiple methods to get current device (some may fail on certain Windows versions)
        commands = [
    ...
    def find_vb_cable_device(self) -> Optional[str]:
        ...
        # Search patterns in priority order: exact match -> partial match -> any CABLE
        search_patterns = [
            ("$_.Name -eq 'CABLE Input (VB-Audio Virtual Cable)'", False),
            ("$_.Name -like 'CABLE Input*' -and $_.Name -like '*VB-Audio*'", False),
            ("$_.Name -like '*CABLE Input*'", True),  # True = show warning
        ]
    ...
    def get_device_index(self, device_name: str) -> Optional[int]:
        ...
        # Escape single quotes in device name
    ...
    def _switch_audio_device(self, device_name: str, verify_contains: Optional[str] = None) -> bool:
        ...
        This is the consolidated device switching logic used by both
        switch_to_vb_cable() and restore_original_device().
        
        Args:
            device_name: Name of the device to switch to
            verify_contains: Optional string that new device name must contain for verification
            
        Returns:
            True if switch was successful, False otherwise
        """
        # Try using device index first (more reliable)
        device_index = self.get_device_index(device_name)
        
        if device_index is not None:
            cmd = f"{self._PS_IMPORT_MODULE}; Set-AudioDevice -Index {device_index}"
        else:
            # Fallback to name-based switching
            escaped_name = device_name.replace("'", "''")
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                # Verify switch if verification string provided
                if verify_contains:
    ...
    def switch_to_vb_cable(self) -> bool:
        ...
        # Check if already set to the target CABLE device
        ...
        if current_device:
            if current_device == self.vb_cable_name:
                ...
                return True
            elif 'CABLE' in current_device.upper() and current_device != self.vb_cable_name:
                _logger.info(f"Found CABLE device but not the target one. Current: {current_device}, Target: {self.vb_cable_name}")
        
        # Auto-detect VB Cable if provided name doesn't exist
